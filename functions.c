...
float sensed_output2 =0;
float control_signal =0;
float control_signal_discrete[2] ={0,0};
float control_signal_discrete2[2] ={0,0};

float last_sensed    =0;
float last_sensed2    =0;
float last_time_inSec=0;
float last_time_inSec2=0;
float error;
float error_array[3]  ;
float error_array2[3] ;
...
/*filter*/
...
#if PIDPARAM ==1
	float Kp         =0.56;     // kp = 0.56 is the parameter with 45d PM, kp = 10 is for test
	...
	float Ki        = 0.345/1;
	float Kd        = 4.68e-07/1;
	#elif PIDPARAM ==12           // this is for test,
	float Kp        = 0.3/9000;   // this one is decent with no hunting phonamena, follows kpkd > ki rule
	float Ki        = 0.0299/90;
	float Kd        = 0.1/9000;
#endif
/******************************************************************************
...
{
  float control_output_increment=0;
...
/******************************************************************************
* Function Name: PID_Discrete2
* Description : same as PID_Discrete, for 2nd test subject
* Arguments :
* Return Value :
******************************************************************************/
float PID_Discrete2(float setpoint,float y,float adjustGain, float timer_inSec)
{
  float control_output_increment=0;
  float current_time = timer_inSec; //returns the number of milliseconds passed since the Arduino started running the program
  ...
  #if PIDMODE ==2 
	float Ti           = Kp/Ki;
  float delta_time = current_time - last_time_inSec2; //delta time interval
	#elif PIDMODE ==3 
	float Ti           = Kp/Ki;
	float Td           = Kd/Kp;
  float delta_time = current_time - last_time_inSec2; //delta time interval
	...
	sensed_output2 = y;  //measured PV
	float target = setpoint;
	error_array2[0] = target - sensed_output2;
//    total_error += error; //accumulates the error - integral term
//    delta_error = error - last_error; //for derivative on error
//    *pdelta_error = -1*(sensed_output - last_sensed); //for derivative on PV
  ...
  #if PIDMODE ==1
	control_output_increment = Kp*adjustGain*(error_array2[0]-error_array2[1]);
  #elif PIDMODE ==2
	control_output_increment = Kp*adjustGain*(error_array2[0]-error_array2[1]+error_array2[0]*delta_time/Ti);
  #elif PIDMODE ==3
	control_output_increment = Kp*adjustGain*(error_array2[0]-error_array2[1]+error_array2[0]*delta_time/Ti+Td*(error_array2[0]-2*error_array2[1]+error_array2[2])/delta_time);
  #elif PIDMODE ==4
	error_array_filter[0] = 0.9*error_array_filter[1]+0.1*error_array[1];
	control_output_increment = Kp*adjustGain*(error_array[0]-error_array[1]+error_array[0]*delta_time/Ti+Td*(error_array_filter[0]-2*error_array_filter[1]+error_array_filter[2])/delta_time);
		...
		control_output_increment = Kp*adjustGain*(error_array[0]-error_array[1]+error_array[0]*delta_time/Ti+Td*(error_array_filter[0]-2*error_array_filter[1]+error_array_filter[2])/delta_time);
  #endif
	control_signal_discrete2[0] = control_signal_discrete2[1] +control_output_increment;

	if (control_signal_discrete2[0]>= max_control) control_signal_discrete2[0] = max_control;
	else if (control_signal_discrete2[0] <= min_control) control_signal_discrete2[0] = min_control;

	error_array2[2] = error_array2[1];
	error_array2[1] = error_array2[0];
	control_signal_discrete2[1] = control_signal_discrete2[0];
	last_time_inSec2 = current_time;
  #if PIDMODE ==4
	error_array_filter[2]=error_array_filter[1];
	error_array_filter[1]=error_array_filter[0];
  #endif
	last_sensed2 = sensed_output; // for derivative on PV
//	int8_t sign_error    = error_array[0]>0?1:(error_array[0] < 0?-1:0);
//  return control_signal_discrete[0]+300*sign_error;
	return control_signal_discrete2[0];
}
/******************************************************************************
...
* Return Value :
******************************************************************************/
//void readADC(ADC_HandleTypeDef *ADC_Handle,uint16_t *ADC_variable, ADC_ChannelConfTypeDef *sConfig)
//{
...
//}
void readADC_multiChannel(ADC_HandleTypeDef *ADC_Handle,uint16_t *ADC_variable, uint8_t NbrOfConversion)
{
	for (uint8_t idx = 0; idx < NbrOfConversion; idx++)
{
    HAL_ADC_Start(ADC_Handle);
    ...
    if(HAL_IS_BIT_SET(HAL_ADC_GetState(ADC_Handle), HAL_ADC_STATE_REG_EOC))
    {
        ...
        ADC_variable[idx] = HAL_ADC_GetValue(ADC_Handle);
    }
}
...
void readADC_singleChannel(ADC_HandleTypeDef *ADC_Handle,uint16_t *ADC_variable)
{
  	  HAL_ADC_Start(ADC_Handle);
    	  if(HAL_ADC_PollForConversion(ADC_Handle, 10)==HAL_OK){
    		*ADC_variable = HAL_ADC_GetValue(ADC_Handle);
...
/******************************************************************************
* Function Name: complexResponse
* Description :  compute complex reeponse
* Arguments :t_s: data sampling period, set 0.001sec, N: N measurements are taken over duration; duration: test duration in sec
* Return Value :
******************************************************mt************************/
void complexResponse(double complex *X_w, double complex *Y_w,float freq, float yin, float yout, float *yinPre, float *youtPre, float t_sec, float t_s, uint16_t *k_stamp, float duration)
{
//double complex z1 = -1*2*M_PI*I;
//double complex z2 = cexp(z1);
...
uint16_t	k = floor(t_sec*1000);  
if (k!= *k_stamp)
	{
//		float yin_modify  = *yinPre+(yin-*yinPre)/(t_sec-t_pre2)* t_s;
//		float yout_modify = *youtPre+(yout-*youtPre)/(t_sec-t_pre2)* t_s;
		float yin_modify = yin;
		float yout_modify= yout;
		float n = freq*duration; // n: wave number
		float N = duration/t_s;
    *X_w +=  yin_modify*cexpf(-1*I*n*2*M_PI*k/N);
    *Y_w +=  yout_modify*cexpf(-1*I*n*2*M_PI*k/N);
...
void complexResponse2(double complex *X_w, double complex *Y_w,float freq, float yin, float yout, float *yinPre, float *youtPre, float t_sec, float t_s, uint16_t *K_stamp, float duration, uint8_t *Initialized)
{
	/* initialize once*/
if(! (*Initialized))
	{
...
/******************************************************************************
* Function Name: complexResponse3
* Description :  compute complex reeponse
* Arguments :t_s: data sampling period, set 0.001sec, N: N measurements are taken over duration; duration: test duration in sec
* Return Value :
******************************************************mt************************/
void complexResponse3(double complex *X_w, double complex *Y_w,float freq, float yin, float yout, float *yinPre, float *youtPre, float t_sec, float t_s, uint16_t *K_stamp, float duration, uint8_t *Initialized)
{
	/* initialize once*/
if(! (*Initialized))
	{
...
//    *Y_w +=  yout_modify*cexpf(-1*I*n*2*M_PI*k/N);
		...
		*X_w += 2/(I*100*N)*yout_modify*cexpf(I*2*M_PI*freq*k);
